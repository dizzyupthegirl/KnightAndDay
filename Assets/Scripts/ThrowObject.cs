...
public class ThrowObject : MonoBehaviour
{
		...
		public void LateUpdate ()
		{
			...
			GetCurrentAnimatorStateInfo (1);
				if (stateInfo.IsName ("UpperBody.Grenade")) {
						if (stateInfo.normalizedTime >= lenghtPrepare * 0.01 &&
								!prepared)
								Prepare ();
						if (stateInfo.normalizedTime >= lenghtThrow * 0.01 &&
								!threw)
								Throw ();
				} else {
						prepared = false;
		...
		{
				prepared = true;
				projectile = Instantiate (projectile, charactersHand.
		                         position, charactersHand.rotation) as GameObject;
